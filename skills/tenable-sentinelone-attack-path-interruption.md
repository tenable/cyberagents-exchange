...
4. Only after naming the host and getting explicit confirmation, isolates the host via SentinelOne's own [`s1-secops-mcp`](https://github.com/Sentinel-One/ai-siem/tree/main/mcp/s1-secops-mcp) server (`purple-mcp` is read-only and has no write tools), then verifies the isolation actually took effect.

Includes `scripts/isolate_agent.py` as a fallback containment call for environments without `s1-secops-mcp` configured. Either path requires confirming the exact endpoint against SentinelOne's current API reference before use — it hasn't been called live in building this skill.
